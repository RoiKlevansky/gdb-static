...
function set_compliation_variables() {
    ...
    if [[ -z "$target_arch" ]]; then
        >&2 echo "Error: target architecture not specified"
        return 1
    elif [[ ! " ${supported_archs[@]} " =~ " ${target_arch} " ]]; then
...
function build_iconv() {
    ...
    ../configure --enable-static "CC=$CC" "CXX=$CXX" "--host=$HOST" 1>&2
    if [[ $? -ne 0 ]]; then
        return 1
    fi
...
function build_libmpfr() {
    ...
    ../configure --enable-static "--with-gmp-build=$libgmp_build_dir" "CC=$CC" "CXX=$CXX" "--host=$HOST" 1>&2
    if [[ $? -ne 0 ]]; then
        return 1
    fi
...
function build_patched_gdb() {
    # Build gdb with a specific patch.
    #
    # Parameters:
...
function build_gdb_with_dependencies() {
    ...
    build_patched_gdb "$packages_dir/gdb" \
                      "$gdb_patch" \
